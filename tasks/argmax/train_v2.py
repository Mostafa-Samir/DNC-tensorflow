...
def generate_data(length, size):
    ...
    return x_seq_list, target_output, weights_vec
        ...
        with tf.Session(graph=graph) as session:
            ...
            for i in xrange(start, end + 1):
                try:
                    ...
                    seq_len = np.random.randint(2, (sequence_max_length / 2) + 1)
                    input_data, target_output, weights = generate_data(seq_len, input_size)
                    ...
                    if summerize:
                        ...
                        print "Real argmax(X): ", str(int(target_output[-1, -1, -1]))
                        print "Predicted f(X): ", str(int(round(output_[-1, -1, -1])))
            ...
            for i in xrange(100):
                ...
                print "\nReal argmax(X): ", str(int(target_output[-1, -1, -1]))
                print "Predicted f(X):   ", str(int(round(output_[-1, -1, -1])))
